...
public class LicenseServiceTest_concludeLicense {
				...
				if (lmgList.get(i).getId().equals("urn:conterra:names:sdi-suite:licensing:licenseset:WFS:FGI_License_Test_20141023")) {
					...
					lmList = lmgList.get(i).getLicenseModels();
		           	...
		           	for (int j = 0; j < lmList.size(); j++) {
		        		...
		        		if (lmList.get(j).getId().equals("urn:conterra:names:sdi-suite:licensing:license:WFS:FGI_License_Test_20141023:LICENSE_WFS_Licensemodel_pekan_concludetesti")) {			
		        			...
		        			for (int k = 0; k < templpList.size(); k++) {
		        				if (templpList.get(k).getParameterClass().equals("configurationParameter")) {			
		        					...
		        					if (templpList.get(k).getName().equals("LICENSE_PARAMETER_Enumeraatioparametri")) {
		        						...
		        						tempParamReplace.addSelection("arvo2");
		        						
		        						templpList.set(k,tempParamReplace);
		        					}
		        					if (templpList.get(k).getName().equals("LICENSE_PARAMETER_multinumeraatio")) {
		        						LicenseParamEnum tempParamReplace = (LicenseParamEnum)templpList.get(k); 
		        						//System.out.println("param.multi "+tempParamReplace.isMulti());
		        						
		        						tempParamReplace.addSelection("hei");
		        						tempParamReplace.addSelection("moi");
